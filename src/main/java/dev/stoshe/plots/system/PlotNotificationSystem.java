...
import dev.stoshe.plots.ui.PlotInfoHUD;
...
public class PlotNotificationSystem extends EntityTickingSystem<EntityStore> {
    ...
    private final Map<UUID, Boolean> lastPageOpen = new HashMap<>();
    ...
    public void tick(float dt, int index, @Nonnull ArchetypeChunk<EntityStore> chunk, @Nonnull Store<EntityStore> store,
        ...
        if (!isAdminBypass && lastId != null && !currentId.equals(lastId) && !lastId.equals("road")) {
            ...
            if (parts.length == 2) {
                try {
                    Plot lastPlot = plotManager.getPlotByGrid(currentWorld.getName(),
        ...
        if (!currentId.equals(lastId) || hudContentChanged) {
            if (plot != null) {
                ...
                PlotConfig.WorldEntry worldView = plotManager.getConfig().world(currentWorld.getName());
                String sizeText = worldView.getPlotSizeX() + "x" + worldView.getPlotSizeZ();

                hud.setPlotInfo(safePlotName, safeOwnerName, sizeText);
                ...
                // Left a plot onto a road: hide the HUD entirely (no plot = no HUD).
                hud.clearPlotInfo();
                safeRequestHudUpdate(hud);
                lastHudState.remove(uuid);
                restoreWorldWeather(playerRef, currentWorld);
                ...
                // On a road (no plot) inside a managed world: hide the HUD entirely.
                hud.clearPlotInfo();
                safeRequestHudUpdate(hud);
                lastHudState.remove(uuid);
                restoreWorldWeather(playerRef, currentWorld);
        ...
        // Hide the HUD while a full-screen plot page (menu / admin / popup) is open,
        // mirroring how the native HUD disappears; restore it when the page closes
        // (polled, so it also covers the native close button).
        boolean pageOpen = isCustomPageOpen(player);
        Boolean wasPageOpen = lastPageOpen.get(uuid);
        if (wasPageOpen == null || wasPageOpen != pageOpen) {
            hud.setSuppressed(pageOpen);
            safeRequestHudUpdate(hud);
            lastPageOpen.put(uuid, pageOpen);
        }
    ...
    private boolean isCustomPageOpen(@Nonnull Player player) {
        try {
            return player.getPageManager() != null && player.getPageManager().getCustomPage() != null;
        } catch (Exception ex) {
            return false;
        }
    }
    ...
    private void teleportToLastSafe(@Nonnull CommandBuffer<EntityStore> buffer, @Nonnull com.hypixel.hytale.component.Ref<EntityStore> ref,
        ...
        buffer.addComponent(ref, Teleport.getComponentType(),
                new Teleport(world, new Vector3d(safePos.x, safePos.y, safePos.z), new Rotation3f(0, 0, 0)));
    }
