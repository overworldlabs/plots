    ...
    protected void executeSync(@Nonnull CommandContext context) {
        ...
        currentWorld.execute(() -> {
            ...
            if (providedGridX != null && providedGridZ != null) {
                ...
                grid = config.getPlotGridAt(currentWorld.getName(), (int) pos.x, (int) pos.z);
            }

            final String worldName = currentWorld.getName();
            Plot plot = this.plotManager.getPlot(worldName, grid[0], grid[1]);
            ...
            PlotConfirmationService.getInstance().request(context.sender(), tm,
                    ...
                    () -> currentWorld.execute(() -> {
                        Plot currentPlot = this.plotManager.getPlot(worldName, targetGridX, targetGridZ);
                        if (currentPlot == null) {
                            playerRef.sendMessage(ChatUtil.error(tm.get("management.not_found")));
                            return;
                        }
                        ...
                        if (this.plotManager instanceof PlotManager pm) {
                            ...
                            deleted = this.plotManager.unclaimPlot(worldName, targetGridX, targetGridZ);
                        }
