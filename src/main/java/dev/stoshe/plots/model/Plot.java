package dev.stoshe.plots.model;

import dev.stoshe.plots.config.PlotConfig;
import dev.stoshe.plots.config.PlotWorldView;
...
public class Plot {
    ...
    /** Name of the plot world this plot belongs to. May be null on legacy data until migrated. */
    ...
    public Plot(int gridX, int gridZ, UUID owner, @Nonnull String ownerName) {
        this(gridX, gridZ, owner, ownerName, (String) null);
    }

    /**
     * Constructs a new Plot bound to a specific plot world.
     *
     * @param world The name of the plot world this plot belongs to
     */
    public Plot(int gridX, int gridZ, UUID owner, @Nonnull String ownerName, String world) {
        this.gridX = gridX;
        this.gridZ = gridZ;
        this.world = world;
        this.owner = owner;
    ...
    /** The plot world this plot belongs to (may be null on un-migrated legacy data). */
    ...
    public int getMinX(PlotConfig config) {
        PlotWorldView v = config.world(world);
        return gridX * (v.getPlotSizeX() + v.getRoadSizeX());
    }
    ...
    public int getMaxX(PlotConfig config) {
        return getMinX(config) + config.world(world).getPlotSizeX();
    }
    ...
    public int getMinZ(PlotConfig config) {
        PlotWorldView v = config.world(world);
        return gridZ * (v.getPlotSizeZ() + v.getRoadSizeZ());
    }
    ...
    public int getMaxZ(PlotConfig config) {
        return getMinZ(config) + config.world(world).getPlotSizeZ();
    }
    ...
    public double getCenterX(PlotConfig config) {
        return getMinX(config) + config.world(world).getPlotSizeX() / 2.0;
    }
    ...
    public double getCenterZ(PlotConfig config) {
        return getMinZ(config) + config.world(world).getPlotSizeZ() / 2.0;
    }
