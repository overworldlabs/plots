...
public class PlotWarpsBrowserPage extends InteractiveCustomUIPage<PlotWarpsBrowserPage.PageData> {
    ...
    /** Warps shown on the current page (parallel to row indices). */
    private List<Plot.PlotWarp> pageView = new ArrayList<>();
    ...
    private static final class Entry {
        ...
        Entry(String label, Plot.PlotWarp warp) {
            this.label = label;
            this.warp = warp;
        }
    }
        ...
        try {
            for (Plot plot : this.plugin.getPlotManager().getAllPlots()) {
                ...
                for (Plot.PlotWarp w : plot.getWarps()) {
                    entries.add(new Entry(plotName + " — " + (w.name != null ? w.name : ""), w));
                }
            }
        ...
        switch (action) {
            ...
            case "WbTp" -> {
                ...
                if (index >= 0 && index < this.pageView.size()) {
                    this.plugin.getPlotManager().teleportPlayerToWarp(store, ref, this.pageView.get(index));
